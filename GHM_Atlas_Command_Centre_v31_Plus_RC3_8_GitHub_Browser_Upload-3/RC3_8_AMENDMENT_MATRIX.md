...
| Responsive navigation close control | Expanded burger displayed a clipped, off-centre X | Corrected the existing `.mobile-menu` and `.burger-lines` geometry so the X is centred and fully contained with 13px effective padding across all six target widths. |
