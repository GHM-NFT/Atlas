...
function markActive(){
  ...
  $$(".territory-shortcut[data-open-territory]").forEach(b=>b.classList.toggle("active",state.view==="territory"&&b.dataset.openTerritory===state.territory));
...
function exportCsv(){
  ...
  const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv"}));a.download="atlas-register-v23.csv";a.click();URL.revokeObjectURL(a.href);toast("Register exported");
}
function init(){
  ...
  $("#addForm").onsubmit=e=>{
    ...
    state.items.push({id,title:f.get("title"),workstream:f.get("workstream"),status:f.get("status"),risk:"Medium",priority:"P2",owner:f.get("owner"),reviewer:"Sophia Reed",progress:20,due:f.get("due"),milestone:"Prototype input",evidence:`Evidence pack ${id}`,decision:`Decision record ${id}`,summary:"New fully populated prototype item added during the v23 experience review.",nextAction:"Review the new item and confirm its dependency path.",dependencies:[]});
