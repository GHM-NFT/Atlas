...
(()=>{
"use strict";
const APP_VERSION="31+ RC3 VISUAL AMENDS";
...
function renderOverview(){
  ...
  $("#overviewTerritoryFocus").innerHTML=`
    <div class="rc2-focus-title">
      <div class="focus-icon-progress"><img src="${w.icon}" alt=""><small>${s.progressKnown?`${s.progress}%`:"Progress not recorded"}</small></div>
  ...
  if(auditHost){
    ...
    auditHost.innerHTML=`<div class="programme-audit-copy">
      <p class="eyebrow">Full programme audit</p>
      <h2>Verified snapshot detail</h2>
      <span>Approved snapshot generated ${dateLabel(meta.exportGenerated)} from the source workbook last modified ${dateLabel(meta.sourceModified)}.</span>
    </div>
    <div class="programme-audit-metrics">
      <div><strong>${meta.totalPhysicalRecords??INTERNAL.allRecords.length}</strong><span>Physical records</span></div>
      <div><strong>${meta.uniqueNodeIds??"—"}</strong><span>Unique node IDs</span></div>
      <div><strong>${meta.workItemCount??state.items.length}</strong><span>Work items</span></div>
      <div><strong>${meta.validationFindingCount??INTERNAL.validationIssues.length}</strong><span>Audit findings</span><small>${(severity.Critical||0)+(severity.High||0)} high or critical</small></div>
    </div>
    <button class="atlas-secondary compact-cta" data-view="internal" type="button"><span>Open full programme audit</span><b aria-hidden="true">→</b></button>`;
  }
...
function drawerHtml(code,tab="overview"){
  ...
  return `<div class="drawer-hero"><div class="drawer-icon-progress"><img class="drawer-main-icon" src="${w.icon}" alt=""><small>${s.progressKnown?`${s.progress}%`:"Progress not recorded"}</small></div><div><p class="eyebrow">${w.code} · Quick Chamber</p><h2>${esc(w.name)}</h2><span>${esc(w.description)}</span></div></div>
...
function renderTerritoryIndex(){
  ...
  $("#territoryIndex").innerHTML=filtered.map(({w,s})=>{
    ...
    return `<article class="territory-card health-state-${hClass}" style="--health:${healthColor}">
      <header>
        <div class="territory-card-title"><div class="territory-card-icon"><img src="${w.icon}" alt=""><small>${s.progressKnown?`${s.progress}%`:"Progress not recorded"}</small></div><div><span class="ws-code">${w.code}</span><h2>${esc(w.name)}</h2><p>${esc(w.description)}</p></div></div>
    ...
    <div class="rc2-territory-layout">
      <section class="rc2-territory-hero" style="--health:${healthColor}">
        ...
        <div class="rc2-territory-hero-side">
          <div class="territory-icon-progress"><img class="rc2-territory-mark" src="${w.icon}" alt=""><small>${s.progressKnown?`${s.progress}%`:"Progress not recorded"}</small></div>
...
function renderWaffle(){
  ...
  $("#waffleFilters").innerHTML=`
    ...
    <div class="waffle-status-icons" role="group" aria-label="Quick status views">
      ${statuses.map(s=>`<button data-waffle-status="${s}" class="${state.waffleStatus===s?"active":""}" data-tip="${s==="all"?"All statuses":s}"><i style="--status:${s==="all"?"#f3f3f3":statusColor[s]}"></i><span>${s==="all"?"All":s}</span></button>`).join("")}
    </div>
  ...
  const aggregateSource=[...state.items.filter(i=>state.waffleWs==="all"||i.workstream===state.waffleWs)].sort((x,y)=>visualOrder[itemVisualState(x)]-visualOrder[itemVisualState(y)]||x.id.localeCompare(y.id));
  const aggregate=`<section class="waffle-overview panel" aria-label="Aggregate delivery waffle">
    <div><p class="eyebrow">Aggregate view</p><h2>${aggregateSource.length} connected work items</h2><span>Select any square to open its quick chamber.</span></div>
    <div class="waffle-overview-grid">${aggregateSource.map(item=>`<button type="button" data-open-item="${item.id}" style="--state:${statusColor[itemVisualState(item)]}" data-tip="${esc(item.id+" · "+item.title+" · "+itemVisualState(item)+" · "+progressLabel(item))}" aria-label="${esc("Open quick chamber for "+item.id+" "+item.title)}"></button>`).join("")}</div>
  </section>`;
  $("#waffleGrid").innerHTML=aggregate+a.map(i=>{
    const w=wsMap.get(i.workstream),visual=itemVisualState(i);
    return `<button class="waffle-item status-${visual.toLowerCase().replace(" ","-")}" data-open-item="${i.id}" style="--state:${statusColor[visual]}" data-tip="${esc(i.id+" · "+i.title+" · "+visual+" · "+progressLabel(i))}">
      <img src="${w.icon}" alt="">
...
function renderMap(){
  ...
  const nodes=visible.map(i=>{
    ...
    return `<circle tabindex="0" class="node-dot ${risk} ${state.mapSelection?.type==="item"&&state.mapSelection.id===i.id?"selected":""}" data-map-item="${i.id}" cx="${p.x}" cy="${p.y}" r="${visual==="Blocked"?9:6.8}" fill="${statusColor[visual]}" style="color:${statusColor[visual]}"><title>${esc(i.id+" · "+i.title+" · "+visual+" · "+progressLabel(i))}</title></circle>${state.labels&&(visual==="Blocked"||i.priority==="P0")?`<text class="node-label" x="${p.x+8}" y="${p.y-7}">${esc(i.id)}</text>`:""}`;
  }).join("");
  const anchors=DATA.workstreams.map(w=>{
    const p=P[w.code],s=summary(w.code),color=s.blocked?"#d30f2f":s.atRisk?"#5d6873":s.review?"#4e8bd8":"#d6b55a";
    return `<g tabindex="0" class="full-map-anchor ${state.mapSelection?.type==="territory"&&state.mapSelection.id===w.code?"selected":""}" data-map-ws="${w.code}" style="--anchor-color:${color}">
      <circle cx="${p.x}" cy="${p.y}" r="64"/>
      <image href="${w.icon}" x="${p.x-60}" y="${p.y-60}" width="120" height="120" preserveAspectRatio="xMidYMid meet"/>
...
function renderInternal(){
    ...
    body=`
      <section class="internal-panel">
        ...
        <div class="internal-entity-grid">
          ...
          <article class="internal-entity-card"><span>Public exposure</span><strong>None</strong><p>This package is intended for approved browser preview use; its embedded data is inspectable by visitors.</p></article>
        </div>
      </section>
      <section class="internal-panel">
  ...
  root.innerHTML=`
    <div class="internal-warning"><i>!</i><div><strong>Browser snapshot build</strong><span>Contains the approved embedded snapshot. GitHub Pages is public; use only where exposure is approved.</span></div><b>READ ONLY · NO WRITE-BACK</b></div>
