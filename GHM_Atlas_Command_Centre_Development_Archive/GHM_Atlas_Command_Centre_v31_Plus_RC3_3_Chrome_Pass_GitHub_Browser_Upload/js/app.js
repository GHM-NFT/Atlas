...
(()=>{
"use strict";
const APP_VERSION="31+ RC3.3 CHROME PASS";
...
function progress(p,color="",known=true){
  ...
  return `<div class="progress compact-progress" ${color?`style="--progress-color:${color}"`:""} aria-label="${safe}% estimated progress"><span class="progress-track"><i style="width:${safe}%"></i></span><b>${safe}%<small> est.</small></b></div>`;
}
...
function renderOverview(){
  ...
  const overviewWaffleItems=[...items].sort((a,b)=>visualOrder[itemVisualState(a)]-visualOrder[itemVisualState(b)]||a.id.localeCompare(b.id));
  const overviewWaffle=$("#overviewWaffleMini");
  if(overviewWaffle){
    overviewWaffle.innerHTML=overviewWaffleItems.map(item=>{
      const visual=itemVisualState(item);
      return `<button data-open-item="${item.id}" type="button" style="--state:${statusColor[visual]}" data-tip="${esc(item.id+" · "+item.title+" · "+visual+" · "+progressLabel(item))}" aria-label="${esc("Open quick chamber for "+item.id+" "+item.title)}"></button>`;
    }).join("");
  }
  const overviewWaffleLegend=$("#overviewWaffleLegend");
  if(overviewWaffleLegend){
    overviewWaffleLegend.innerHTML=["Blocked","At risk","Review","Active","Later","Completed"].map(status=>`<span><i style="--state:${statusColor[status]}"></i>${status}</span>`).join("");
  }
...
function renderOverviewMap(){
  ...
  const anchors=DATA.workstreams.map(w=>{
    ...
    return `<g class="overview-anchor" data-map-ws="${w.code}" style="--anchor-color:${s.blocked?"#ff1744":s.atRisk?"#ff8a00":"#d6b55a"}">
      <image href="${w.icon}" x="${p.x-50}" y="${p.y-50}" width="100" height="100" preserveAspectRatio="xMidYMid meet"/>
  ...
  const center=`<g class="overview-centre" data-view="overview"><image href="assets/brand/command-centre-map-icon.png" x="724" y="394" width="152" height="152" preserveAspectRatio="xMidYMid meet"/></g>`;
...
function renderCondensedTerritories(){
  ...
  el.innerHTML=DATA.workstreams.map(w=>{
    ...
    return `<button class="territory-condensed" data-open-territory="${w.code}">
      ...
      <div class="territory-condensed-metrics"><span>${s.active} active · ${s.blocked} blocked</span>${progress(s.progress,"",s.progressKnown)}</div>
    </button>`;
  }).join("");
}
    ...
    <div class="rc2-territory-layout">
      <section class="rc2-territory-hero" style="--health:${healthColor}">
        ...
        <div class="rc2-territory-hero-side">
          <div class="territory-icon-progress">
