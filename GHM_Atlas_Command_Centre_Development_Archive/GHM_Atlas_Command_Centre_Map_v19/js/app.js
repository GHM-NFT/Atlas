(() => {
  ...
  const el = {
    ...
    visualTooltip: $("#visualMapTooltip"), visualFocusHint: $("#visualFocusHint"), detailSummaryBand: $("#detailSummaryBand")
  };
  ...
  function handleVisualAnchorClick(code) {
    ...
    state.visualRelationFocus=null;
    state.visualWorkstreamFocus=code;
    renderVisualMap(filteredNodes());
  }

  function updateVisualFocusHint(){
    if(!el.visualFocusHint)return;
    if(state.visualFocus){
      const n=state.nodes.find(x=>x.id===state.visualFocus);
      el.visualFocusHint.textContent=n ? `Focused node: ${shorten(n.title,42)} · click again to clear` : "Focused dependency neighbourhood";
    } else if(state.visualWorkstreamFocus){
      const w=WORKSTREAMS.find(x=>x.code===state.visualWorkstreamFocus);
      el.visualFocusHint.textContent=w ? `${w.code} focused · click the icon again to open details` : "Focused workstream";
    } else {
      el.visualFocusHint.textContent="Click once to focus · click again to open";
    }
  }

  function renderVisualMap(nodes){
    updateVisualFocusHint();
    ...
    const edges=state.edges.map(e=>{
      ...
      const visible=state.visualEdges||highlight;
      const relationFocused=state.visualRelationFocus===e.id;
      return `<g class="vm-edge-wrap ${relationFocused?"relation-focused":""}" data-vm-edge="${escapeHtml(e.id)}">
        <path class="vm-edge-hit" d="${visualCurve(a,b)}"></path>
        <path class="vm-edge ${visible?"visible":""} ${highlight?"highlight":""} ${dim?"dim":""} ${blocked?"blocked":""}" d="${visualCurve(a,b)}"></path>
      </g>`;
    }).join("");
    ...
    const focusTitle=state.visualRelationFocus
      ? `Dependency relation isolated`
      : state.visualWorkstreamFocus
  ...
  function bindVisualMap(layout){
    ...
    $$("[data-vm-edge]").forEach(g=>{
      const edge=state.edges.find(e=>e.id===g.dataset.vmEdge);
      if(!edge)return;
      g.addEventListener("click",e=>{
        e.stopPropagation();
        state.visualRelationFocus=state.visualRelationFocus===edge.id?null:edge.id;
        state.visualFocus=null;
        state.visualWorkstreamFocus=null;
        renderVisualMap(filteredNodes());
      });
      bindVisualTooltip(g,()=>visualEdgeTip(edge));
    });
  ...
  function visualEdgeTip(edge){
    const map=visualNodeMap(),a=map.get(edge.source),b=map.get(edge.target);
    return `<strong>${escapeHtml(edge.type||"Dependency")}</strong>
      ${escapeHtml(a?.title||edge.source)} → ${escapeHtml(b?.title||edge.target)}
      ${edge.label?`<div class="vm-edge-label">${escapeHtml(edge.label)}</div>`:""}
      <div class="vm-tip-meta"><span>Click to isolate relation</span></div>`;
  }
  ...
  function resetVisualMap(){
    ...
    state.visualFocus=null;state.visualWorkstreamFocus=null;state.visualRelationFocus=null;
