...
(()=>{
"use strict";
const APP_VERSION="31+ RC3.1 RESPONSIVE CORRECTIONS";
...
const statusColor={Completed:"#34b56f",Active:"#d99a3c",Review:"#4e8bd8",Blocked:"#d30f2f",Later:"#8d969f","At risk":"#b9283e"};
...
const progressLabel=i=>knownProgress(i)?`${Math.round(Number(i.progress))}%${i.progressProvisional?" provisional":""}`:"Not recorded";
const summaryProgressLabel=s=>s?.progressKnown?`${Math.round(Number(s.progress))}%${s.progressProvisional?" provisional":""}`:"Not recorded";
...
function riskChip(risk){const c={Low:"#7c8790",Medium:"#d99a3c",High:"#d7483f",Critical:"#d30f2f"}[risk];return `<span class="risk-chip" style="--risk:${c}">${esc(risk)}</span>`}
...
function installRouteBackButtons(){
  $$(".shared-command-banner").forEach(banner=>{
    if(banner.querySelector(".route-back"))return;
...
function renderOverview(){
  ...
  const h=health(s);
  const healthColor=h==="Critical"?"#d30f2f":h==="Attention"?"#d99a3c":h==="On track"?"#34b56f":"#4e8bd8";
  ...
  if(auditHost){
    ...
    auditHost.innerHTML=`
      ...
      <article class="internal-summary-card" style="--metric:#5d6873"><span>Progress field</span><strong>50%</strong><small>Provisional placeholder</small></article>`;
  }
...
function renderOverviewMap(){
  ...
  const dots=state.items.map(i=>{
    ...
    return `<circle class="overview-dot ${riskOrder[i.risk]>=2?"risk":""}" data-map-item="${i.id}" cx="${p.x}" cy="${p.y}" r="${visual==="Blocked"?7.2:5}" fill="${statusColor[visual]}"><title>${esc(i.id+" · "+i.title+" · "+visual)}</title></circle>`;
  }).join("");
  const anchors=DATA.workstreams.map(w=>{
    const p=P[w.code],s=summary(w.code);
    return `<g class="overview-anchor" data-map-ws="${w.code}" style="--anchor-color:${s.blocked?"#d30f2f":s.atRisk?"#5d6873":"#d6b55a"}">
      <circle cx="${p.x}" cy="${p.y}" r="53"/>
      <image href="${w.icon}" x="${p.x-44}" y="${p.y-44}" width="88" height="88" preserveAspectRatio="xMidYMid meet"/>
...
function renderCondensedTerritories(){
  ...
  el.innerHTML=DATA.workstreams.map(w=>{
    ...
    return `<button class="territory-condensed" data-open-territory="${w.code}">
      ...
      <div class="territory-condensed-metrics"><b>${summaryProgressLabel(s)}</b><span>${s.active} active · ${s.blocked} blocked</span>${progress(s.progress,"",s.progressKnown)}</div>
    </button>`;
  }).join("");
}
...
function drawerHtml(code,tab="overview"){
  ...
  if(tab==="owners"){
    ...
    body=grouped.map(([owner,arr])=>{
      ...
      return `<div class="row-item"><span class="status-chip">${arr.length}</span><div><strong>${esc(owner)}</strong><br><span>${arr.filter(i=>i.status==="Blocked").length} blocked · ${value===null?"progress not recorded":`${value}% provisional average`}</span></div><span>Lead</span></div>`;
    }).join("");
  }
...
function territoryNarrative(code,s){
  ...
  if(!s.progressKnown)return `${w.name} has ${s.total} mapped work items. A provisional visual percentage is shown pending the next approved Command Sheet snapshot.`;
...
function territoryMiniMap(code,items,healthColor){
  ...
  const nodes=selected.map(item=>{
    ...
    return `<circle class="mini-node" data-open-item="${item.id}" cx="${p.x}" cy="${p.y}" r="${visual==="Blocked"?7.5:5.5}" fill="${statusColor[visual]}" data-tip="${esc(item.id+" · "+item.title+" · "+visual+" · "+progressLabel(item))}"><title>${esc(item.id+" · "+item.title+" · "+visual)}</title></circle>`;
  }).join("");
  const w=wsMap.get(code);
  return `<svg class="territory-bird-mini" viewBox="0 0 600 330" role="img" aria-label="${esc(w.name)} territory Bird’s-eye preview">
    ${links.join("")}
    <circle class="mini-anchor" style="--health:${healthColor}" cx="${cx}" cy="${cy}" r="72"/>
    <image href="${w.icon}" x="${cx-66}" y="${cy-66}" width="132" height="132" preserveAspectRatio="xMidYMid meet"/>
    ${nodes}
  </svg>`;
}
    ...
    <div class="rc2-territory-layout">
      <section class="rc2-territory-hero" style="--health:${healthColor}">
        ...
        <div class="rc2-territory-hero-side">
          <div class="territory-icon-progress"><img class="rc2-territory-mark" src="${w.icon}" alt=""><small>${summaryProgressLabel(s)}</small></div>
          <div class="rc2-territory-status">
            <span>Current status</span>
            <strong>${h}</strong>
            <small>${s.progressProvisional?"Provisional progress":s.progressKnown?"Recorded progress":"Progress not recorded"}</small>
      ...
      <section>
        <div class="overview-section-title">
          ...
          <span>Progress percentages are provisional visual placeholders pending the next approved snapshot.</span>
        </div>
      ...
      <section class="rc2-territory-primary">
        ...
        <article class="panel rc2-territory-waffle">
          ...
          <div class="rc2-territory-waffle-mini">${waffleItems.map(item=>`<button data-open-item="${item.id}" style="--state:${statusColor[itemVisualState(item)]}" data-tip="${esc(item.id+" · "+item.title+" · "+itemVisualState(item)+" · "+progressLabel(item))}" aria-label="${esc("Open quick chamber for "+item.id+" "+item.title)}"></button>`).join("")}</div>
...
function renderWaffle(){
  ...
  const aggregateSource=[...state.items.filter(i=>state.waffleWs==="all"||i.workstream===state.waffleWs)]
    .sort((x,y)=>visualOrder[itemVisualState(x)]-visualOrder[itemVisualState(y)]||x.id.localeCompare(y.id));
  const spanFor=id=>{
    let h=0;
  ...
  const aggregateGroups=deliveryStates.map(status=>{
    const records=aggregateSource.filter(item=>itemVisualState(item)===status);
    if(!records.length)return"";
    return `<section class="waffle-mass-cluster" style="--state:${statusColor[status]}">
      <header><strong>${status}</strong><span>${records.length}</span></header>
      <div class="waffle-stroke-field">${records.map(item=>`<button class="waffle-stroke" type="button" data-open-item="${item.id}" style="--stroke-span:${spanFor(item.id)}" data-tip="${esc(item.id+" · "+item.title+" · "+status+" · "+progressLabel(item))}" aria-label="${esc("Open quick chamber for "+item.id+" "+item.title)}"></button>`).join("")}</div>
    </section>`;
  }).join("");
  const aggregate=`<section class="waffle-overview panel" aria-label="Aggregate delivery waffle">
    <div class="waffle-overview-copy"><p class="eyebrow">Aggregate view</p><h2>${aggregateSource.length} connected work items</h2><span>Grouped colour masses show the scale of each delivery state. Select any stroke to open its Quick Chamber.</span></div>
    <div class="waffle-mass">${aggregateGroups}</div>
  </section>`;
  ...
  const grouped=deliveryStates.map(status=>{
    ...
    return `<section class="waffle-status-group" style="--state:${statusColor[status]}">
      ...
      <div class="waffle-ribbon-grid">${records.map(item=>{
        const w=wsMap.get(item.workstream);
        return `<button class="waffle-item status-${status.toLowerCase().replace(" ","-")}" data-open-item="${item.id}" style="--state:${statusColor[status]}" data-tip="${esc(item.id+" · "+item.title+" · "+status+" · "+progressLabel(item))}" type="button">
          <span class="waffle-item-stroke" aria-hidden="true"></span>
          <img src="${w.icon}" alt="">
          <span class="waffle-item-copy"><b>${item.id} · ${item.workstream}</b><strong>${esc(short(item.title,72))}</strong><small>${esc(item.owner||"Unassigned")}</small></span>
          <span class="waffle-item-meta"><em>${status}</em><strong>${progressLabel(item)}</strong></span>
        </button>`;
      }).join("")}</div>
    </section>`;
  }).join("");
...
function positions(layout,compact=window.innerWidth<=970){
  const P={};
  const desktopCentre={x:800,y:470};
  const compactCentre={x:450,y:1180};
  ...
  const curatedCompact={
    WS001:{x:150,y:150},WS002:{x:450,y:150},WS003:{x:750,y:150},
    WS004:{x:150,y:430},WS005:{x:450,y:430},WS006:{x:750,y:430},
    WS007:{x:150,y:710},WS008:{x:450,y:710},WS009:{x:750,y:710},
    WS010:{x:300,y:990},WS011:{x:600,y:990}
  };
  DATA.workstreams.forEach((w,wi)=>{
    ...
    if(compact){
      ...
      ax=235+(wi%4)*375;ay=155+Math.floor(wi/4)*315;
    }else if(layout==="workflow"){
      ax=150+(wi%6)*260;ay=220+Math.floor(wi/6)*440;
    }else{
    ...
    arr.forEach((item,index)=>{
      ...
      if(compact){
        ...
        const radius=50+(index%4)*12;
        x=ax+Math.cos(angle)*radius;
        y=ay+Math.sin(angle)*radius;
      }else if(layout==="workflow"){
        x=ax+((index%6)-2.5)*31;y=ay+68+Math.floor(index/6)*29;
      }else if(layout==="territory"){
        x=ax+((index%6)-2.5)*29;y=ay+65+Math.floor(index/6)*28;
      }else{
        const angle=-Math.PI/2+index*Math.PI*2/Math.max(1,arr.length)+(wi*.17);
        const radius=76+(index%4)*18;
        x=ax+Math.cos(angle)*radius;
        y=ay+Math.sin(angle)*radius;
      }
      P[item.id]={x,y};
...
function renderMap(){
  ...
  const dimensions=compact?{width:900,height:1240,cx:450,cy:1180}:{width:1600,height:940,cx:800,cy:470};
  ...
  const nodes=visible.map(item=>{
    ...
    const radius=visual==="Blocked"?(compact?10:9):(compact?8:6.8);
    return `<circle tabindex="0" role="button" class="node-dot ${risk} ${state.mapSelection?.type==="item"&&state.mapSelection.id===item.id?"selected":""}" data-map-item="${item.id}" cx="${p.x}" cy="${p.y}" r="${radius}" fill="${statusColor[visual]}" style="color:${statusColor[visual]}"><title>${esc(item.id+" · "+item.title+" · "+visual+" · "+progressLabel(item))}</title></circle>${state.labels&&(visual==="Blocked"||item.priority==="P0")?`<text class="node-label" x="${p.x+10}" y="${p.y-8}">${esc(item.id)}</text>`:""}`;
  }).join("");

  const anchorRadius=compact?50:64;
  const iconSize=compact?88:120;
  const anchors=DATA.workstreams.map(workstream=>{
    const p=P[workstream.code];
    const summaryData=summary(workstream.code);
    const color=summaryData.blocked?"#d30f2f":summaryData.atRisk?"#5d6873":summaryData.review?"#4e8bd8":"#d6b55a";
    return `<g tabindex="0" role="button" aria-label="${esc("Open "+workstream.name+" Quick Chamber")}" class="full-map-anchor ${state.mapSelection?.type==="territory"&&state.mapSelection.id===workstream.code?"selected":""}" data-map-ws="${workstream.code}" style="--anchor-color:${color}">
      <circle cx="${p.x}" cy="${p.y}" r="${anchorRadius}"/>
  ...
  const centreSize=compact?104:144;
  const center=`<g class="map-centre" data-map-home role="button" tabindex="0"><circle cx="${dimensions.cx}" cy="${dimensions.cy}" r="${compact?60:82}"/><image href="assets/brand/command-centre-emblem.png" x="${dimensions.cx-centreSize/2}" y="${dimensions.cy-centreSize/2}" width="${centreSize}" height="${centreSize}" preserveAspectRatio="xMidYMid meet"/><title>Return to Overview</title></g>`;
