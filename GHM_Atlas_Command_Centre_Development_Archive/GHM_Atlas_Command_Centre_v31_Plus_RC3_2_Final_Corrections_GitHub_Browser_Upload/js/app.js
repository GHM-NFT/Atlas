...
(()=>{
"use strict";
const APP_VERSION="31+ RC3.2 FINAL CORRECTIONS";
...
const statusColor={Completed:"#34b56f",Active:"#d99a3c",Review:"#4e8bd8",Blocked:"#ff1744",Later:"#8d969f","At risk":"#ff8a00"};
...
const progressValueHtml=i=>knownProgress(i)?`<span>${Math.round(Number(i.progress))}%</span>${i.progressProvisional?"<small>provisional</small>":""}`:"<span>—</span><small>not recorded</small>";
...
function progress(p,color="",known=true){
  ...
  if(known===false||!Number.isFinite(value)){
    return `<div class="progress unknown" ${color?`style="--progress-color:${color}"`:""} aria-label="Progress not recorded"><span class="progress-track"><i></i></span><b>—</b></div>`;
  }
  const safe=Math.max(0,Math.min(100,Math.round(value)));
  return `<div class="progress compact-progress" ${color?`style="--progress-color:${color}"`:""} aria-label="${safe}% provisional progress"><span class="progress-track"><i style="width:${safe}%"></i></span><b>${safe}%</b></div>`;
}
...
function renderOverviewMap(){
  ...
  const anchors=DATA.workstreams.map(w=>{
    ...
    return `<g class="overview-anchor" data-map-ws="${w.code}" style="--anchor-color:${s.blocked?"#ff1744":s.atRisk?"#ff8a00":"#d6b55a"}">
...
function territoryMiniMap(code,items,healthColor){
  ...
  const nodes=selected.map(item=>{
    ...
    return `<g class="mini-node-action" tabindex="0" role="button" data-open-item="${item.id}" aria-label="${esc("Open quick chamber for "+item.id+" "+item.title)}" data-tip="${esc(item.id+" · "+item.title+" · "+visual+" · "+progressLabel(item))}">
      <circle class="mini-node-hit" cx="${p.x}" cy="${p.y}" r="16"/>
