...
function renderSquare(){
  ...
  $("#blockViewControls").innerHTML=`
    ...
    <div class="block-status-strip">
      <button data-block-status="all" class="${state.blockStatus==="all"?"active":""}"><span>All blocks</span><strong>${state.items.length}</strong></button>
      ${counts.map(x=>`<button data-block-status="${x.status}" class="status-${x.status.toLowerCase()} ${state.blockStatus===x.status?"active":""}"><span>${x.status}</span><strong>${x.count}</strong></button>`).join("")}
    </div>`;
  ...
  if(state.blockMode==="overview"){
    ...
    $("#squareGrid").innerHTML=`<section class="project-block-field">
      ...
      <div class="project-block-grid">${visible.map(i=>`<button class="project-block status-${i.status.toLowerCase()}" data-open-item="${i.id}" style="--state:${statusColor[i.status]}" title="${esc(i.id+" · "+i.title+" · "+i.status+" · "+i.progress+"%")}" aria-label="${esc(i.id+" "+i.title+", "+i.status+", "+i.progress+" percent")}"><span>${i.id}</span></button>`).join("")}</div>
...
function init(){
  ...
  const hideLoader=()=>{
    const remaining=Math.max(0,650-(performance.now()-loaderStart));
