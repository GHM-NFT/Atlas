...
const state={
  ...
  waffleStatus:"all",waffleWs:"all",registerStatus:"all",registerWs:"all",
...
function markActive(){
  ...
  $$("[data-open-ws]").forEach(b=>b.classList.toggle("active",state.view==="territory"&&b.dataset.openWs===state.territory));
}
...
function renderOverview(){
  ...
  $("#workstreams").innerHTML=DATA.workstreams.map(w=>{
    ...
    return `<article class="ws-card">
      <button class="ws-card-main" data-open-territory="${w.code}">
      ...
      <button class="ws-quick" data-open-ws="${w.code}">Quick chamber</button>
    </article>`;
  }).join("");
  const priority=all.filter(i=>i.status==="Blocked"||i.priority==="P0"||i.risk==="Critical")
    .sort((a,b)=>riskOrder[b.risk]-riskOrder[a.risk]||a.due.localeCompare(b.due)).slice(0,8);
...
function renderSquare(){
  $("#squareGrid").innerHTML=DATA.workstreams.map(w=>{
    ...
    return `<section class="block-card">
      <button class="block-head text-only" data-open-territory="${w.code}"><div><span>${w.code}</span><h3>${esc(w.name)}</h3><small>${health(s)} · ${esc(w.description)}</small></div></button>
...
function renderWaffle(){
  ...
  $("#waffleGrid").innerHTML=a.map(i=>`<button class="waffle-item no-icon" data-open-item="${i.id}" style="--state:${statusColor[i.status]}" title="${esc(i.summary)}">
      <b>${i.id} · ${i.workstream}</b><strong>${esc(short(i.title,47))}</strong><span>${i.status} · ${i.progress}% · ${esc(i.owner)}</span>${progress(i.progress)}
    </button>`).join("");
...
function renderMap(){
  const svg=$("#birdMap"),P=positions(state.mapLayout),m=itemMap(),status=$("#mapStatus").value;
  const visible=state.items.filter(i=>status==="all"||i.status===status);
  let edges="";
  if(state.edges){
    visible.forEach(i=>i.dependencies.forEach(d=>{
      if(!P[d])return;const target=m.get(d),color=target?statusColor[target.status]:"#aeb5bb";
  ...
  DATA.workstreams.forEach(w=>{
    ...
    labels+=`<g class="territory-map-label" data-map-ws="${w.code}"><rect x="${p.x-84}" y="${p.y-18}" rx="9" width="168" height="36"/><text x="${p.x}" y="${p.y-2}">${w.code}</text><text class="sub" x="${p.x}" y="${p.y+11}">${esc(short(w.name,25))}</text><title>${esc(w.name)} · ${s.total} items · ${s.blocked} blocked</title></g>`;
  });
  ...
  svg.innerHTML=`<g id="mapLayer">${edges}${nodes}${labels}${center}</g>`;
...
function exportCsv(){
  ...
  const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([csv],{type:"text/csv"}));a.download="atlas-register-v22.csv";a.click();URL.revokeObjectURL(a.href);toast("Register exported");
}
function init(){
  ...
  $("#addForm").onsubmit=e=>{
    ...
    state.items.push({id,title:f.get("title"),workstream:f.get("workstream"),status:f.get("status"),risk:"Medium",priority:"P2",owner:f.get("owner"),reviewer:"Sophia Reed",progress:20,due:f.get("due"),milestone:"Prototype input",evidence:`Evidence pack ${id}`,decision:`Decision record ${id}`,summary:"New fully populated prototype item added during the v22 experience review.",nextAction:"Review the new item and confirm its dependency path.",dependencies:[]});
