(() => {
  ...
  const el = {
    ...
    visualControls: $("#visualMapControls"), visualLayout: $("#visualLayout"),
  ...
  function visualLayoutData(nodes, mode) {
    const W=1800,H=1120,cx=W/2,cy=H/2;
  ...
  function renderVisualMap(nodes){
    ...
    const related=state.visualFocus?visualRelatedIds(state.visualFocus):null;

    const edges=state.edges.map(e=>{
      const a=layout.positions.get(e.source),b=layout.positions.get(e.target);
      if(!a||!b)return"";
      const highlight=state.visualFocus&&(e.source===state.visualFocus||e.target===state.visualFocus);
      const dim=state.visualFocus&&!highlight;
    ...
    const dots=nodes.map(n=>{
      ...
      const dep=visualDepCounts(n.id), selected=state.visualFocus===n.id;
      const dim=related&&!related.has(n.id), radius=Math.min(11,4+Math.sqrt(dep.total)*1.6);
      return `<g class="vm-node ${dim?"dim":""}" tabindex="0" data-vm-node="${escapeHtml(n.id)}" transform="translate(${p.x},${p.y})">
        <circle class="vm-risk ${n.risk}" r="${radius+3}"></circle>
        <circle class="vm-dot ${n.state}" r="${radius}"></circle>
        <text class="vm-label" x="${radius+7}" y="4">${escapeHtml(shorten(n.title,46))}</text>
    ...
    const anchors=state.visualIcons?WORKSTREAMS.map(w=>{
      ...
      const s=summarise(nodes,w.code), selected=state.selectedWorkstream===w.code;
      return `<g class="vm-anchor ${selected?"selected":""}" tabindex="0" data-vm-anchor="${w.code}" transform="translate(${p.x},${p.y})">
        <circle class="vm-anchor-halo" r="58"></circle>
        <image class="vm-anchor-image" href="assets/workstream-icons/${w.icon}-responsive.png" x="-45" y="-45" width="90" height="90"></image>
        <text class="vm-anchor-code" y="76">${w.code}</text>
        <text class="vm-anchor-title" y="94">${escapeHtml(shorten(w.name,26))}</text>
      </g>`;
    }).join(""):"";
    ...
    el.viewContainer.innerHTML=`<div class="visual-map-stage"><svg class="visual-map-svg ${state.visualLabels?"show-labels":""}" viewBox="0 0 ${layout.W} ${layout.H}" aria-label="Atlas visual project map">
      <g class="vm-world" transform="translate(${state.visualX} ${state.visualY}) scale(${state.visualScale})">
        ${titles}${edges}${dots}${anchors}
      </g>
    </svg></div>`;
  ...
  function bindVisualMap(layout){
    ...
    $$("[data-vm-node]").forEach(g=>{
      g.addEventListener("click",()=>{state.visualFocus=state.visualFocus===g.dataset.vmNode?null:g.dataset.vmNode;renderVisualMap(filteredNodes())});
    ...
    $$("[data-vm-anchor]").forEach(g=>{
      g.addEventListener("click",()=>showDetails(g.dataset.vmAnchor));
      g.addEventListener("keydown",e=>{if(e.key==="Enter"||e.key===" "){e.preventDefault();g.click()}});
  ...
  function visualWorkstreamTip(code){
    ...
    return `<strong>${code} — ${escapeHtml(w.name)}</strong>${s.done} complete · ${s.open} open · ${s.blocked} blocked
      <div class="vm-tip-meta"><span>${s.percent}% complete</span><span>${s.risk} at risk</span></div>`;
  }
  ...
  function resetVisualMap(){
    state.visualScale=1;state.visualX=0;state.visualY=0;state.visualFocus=null;
  ...
  $("#resetVisualFocus").addEventListener("click",()=>{state.visualFocus=null;if(state.view==="visualmap")renderVisualMap(filteredNodes())});
