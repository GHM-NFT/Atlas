(() => {
  ...
  function updateCounts() {
    ...
    el.totalItems.textContent=`${nodes.length} items`;
  }

  function update(){
    updateCounts();
    const nodes=filteredNodes();
    el.resultsSummary.textContent=`Showing ${nodes.length} of ${state.nodes.length} live items`;
  ...
  function renderWaffle(nodes){
    ...
    const cells=capped.map(n=>`<button class="waffle-cell ${n.state.toLowerCase()}" data-workstream="${n.workstream}" title="${escapeHtml(n.title)} — ${n.state}"></button>`).join("");
  ...
  function showDetails(code){
    ...
    $("#detailList").innerHTML=s.group.slice(0,30).map(n=>`<div class="detail-item ${n.state.toLowerCase()}"><strong>${escapeHtml(n.title)}</strong><small>${n.state} · ${n.risk} risk · ${escapeHtml(n.owner)}</small></div>`).join("") || '<div class="empty-state">No matching items</div>';
